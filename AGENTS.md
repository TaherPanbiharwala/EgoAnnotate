...
| Stage | Status |
|---|---|
| EgoBlur redaction | Heavily built, heavily tested, heavily reviewed. One real clip (`GX010057`) has been run three times while tuning parameters. **The fill-integrity question that used to gate scaling is resolved** — see "Immediate priority" below. The pre-redaction PoseLandmarker prior is permanently shadow-only. The only behavior-changing experimental option is `--hand-suppress-wearer-hands`, backed by a separate private MediaPipe Hand Landmarker artifact: it can withhold only a repeated face track whose every raw box is nearly fully inside the same stable, close camera-wearer hand. Active mode is pinned to 10 Hz and the approved Hand Landmarker SHA-256, and rejects stale policy settings. It never becomes a detector ROI, never reduces detection on other people, records private evidence, and a nonzero count forces `NEEDS_REVIEW`. A batch must use a private report directory so every clip retains its suppression evidence. One known, lower-urgency gap remains (resumed-batch config drift, see "Known open work") before an unattended 16-clip run. |
