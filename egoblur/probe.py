...
Vendored by copy from pipeline/src/egoannote/media/probe.py so that egoblur/
stays fully self-contained (same reason egoblur/job.py vendors rather than
imports: this folder must not depend on the pipeline/ package or its venv).
Keep this in sync by hand if the original changes.
"""
