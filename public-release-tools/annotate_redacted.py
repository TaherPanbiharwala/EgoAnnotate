"""MediaPipe hands + dense VLM captioning on an EgoBlur-redacted video, plus
Hugging Face publishing. Extracted from egoannote.pipeline's old `annotate`/
`publish-hf`/`approve-redaction` commands when the active pipeline moved to
curate-original/annotate-curated-original. Kept working as a reference — see
public-release-tools/README.md.

The only video accepted by the annotation stages here is ``redacted_video``;
``original_video`` is registered for later private perception stages and
Drive archiving but is never decoded by MediaPipe or sent to a VLM.
"""
...
from egoannote.pipeline import (
    _atomic_json,
    _file_binding,
    _load_json,
    _read_private_manifest,
    _save_private_manifest,
    _sha256_file,
    _utc_now,
    _validate_video_id,
)
